...
function generateStage()
{
	...
	var bgSchool:FNFSprite = new FNFSprite(repositionShit, 0).loadGraphic(Paths.image('weebSchool', stageDir));
	bgSchool.scrollFactor.set(0.6, 0.90);
	add(bgSchool);

	var bgStreet:FNFSprite = new FNFSprite(repositionShit).loadGraphic(Paths.image('weebStreet', stageDir));
	bgStreet.scrollFactor.set(0.95, 0.95);
	add(bgStreet);

	var fgTrees:FNFSprite = new FNFSprite(repositionShit + 170, 130).loadGraphic(Paths.image('weebTreesBack', stageDir));
