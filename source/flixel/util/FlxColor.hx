...
abstract FlxColor(Int) from Int from UInt to Int to UInt
{
	...
	/**
	 * Generate a color from float RGB values (0 to 255).
	 * Values will be converted from floats to integers.
	 *
	 ...
	 * @return The color as a FlxColor
	 */
	public static inline function fromRGB(Red:Float, Green:Float, Blue:Float, Alpha:Float = 255):FlxColor
	{
		var color = new FlxColor();
		return color.setRGB(Std.int(Red), Std.int(Green), Std.int(Blue), Std.int(Alpha));
	}
