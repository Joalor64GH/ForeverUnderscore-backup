 ...
 * @origin Psych Engine
 */
class PsychDropDown extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
	...
	public function new(X:Float = 0, Y:Float = 0, DataList:Array<StrNameLabel>, ?Callback:String->Void, ?Header:FlxUIDropDownHeader,
			?DropPanel:FlxUI9SliceSprite, ?ButtonList:Array<FlxUIButton>, ?UIControlCallback:Bool->PsychDropDown->Void)
	{
		super(X, Y);
		callback = Callback;
		header = Header;
	...
	/*public function setUIControlCallback(UIControlCallback:Bool->PsychDropDown->Void):Void {
		_ui_control_callback = UIControlCallback;
	}*/
	 ...
	 * @return	The StrIDLabel array ready to be used in PsychDropDown's constructor
	 */
...
class FlxUIDropDownHeader extends FlxUIGroup
{
	...
	/**
	 * Creates a new dropdown header to be used in a PsychDropDown.
	 *
